$(document).ready(()=>{
    $("#app-controls-close").on("click", ()=>{
        $.post("/app/controls/app-controls-close");
    });
    $("#app-controls-settings").on("click", (e)=>{
        window.LoadClasses("assets/overlay/classes.json").then(()=>{
            window.LoadOverlay(AppOverlay_AppConfig, window.MakeData());
        });
    });
    $("#app-controls-hide").on("click", ()=>{
        $.post("/app/controls/app-controls-hide");
    });
    $("#app-controls-support").on("click", (e)=>{
        window.LoadClasses("assets/overlay/classes.json").then(()=>{
            window.LoadOverlay(AppOverlay_AboutInfo, window.MakeData());
        });
    });
    $("#app-controls-size").on("click", ()=>{
    ...
    $("#app-nav-profile-chart").on("click", (e)=>{
        Promise.all([
            window.LoadClasses("assets/body/side/pages/summoner/classes.json"),
            window.LoadClasses("assets/body/main/pages/summoner/classes.json"),
        ]).then(()=>{
            if(SummonerIdentifier.currentIdentifier && !SummonerIdentifier.currentIdentifier.invalid){
                window.LoadMain(Main_Summoner, window.MakeData({
                    identifier:{
                ...
                SummonerIdentifier.CurrentSummoner().then((identifier)=>{
                    window.LoadPage(Side_Summoner, Main_Summoner, window.MakeData({
                        identifier:{
                            "name":"summoner-recent-performance", 
                            "class": Main_Part_SummonerRecentPerformance,
                        },
