...
window.LoadHomePage = (()=>Promise.all([
    window.LoadClasses("assets/body/side/pages/summoner/classes.json")
        .then(()=>SummonerIdentifier.CurrentSummoner())
        .then((identifier)=>window.LoadSide(Side_Summoner, window.MakeData({summoner:identifier}))),
    window.LoadClasses("assets/body/main/pages/lol-match-history/classes.json")
        .then(()=>SummonerIdentifier.CurrentSummoner())
        .then((identifier)=>window.LoadMain(Main_LolMatchHistory, window.MakeData({summoner:identifier}))),
]));
...
$(document).ready(function(){
    ...
    $.get("/app/config/options", {}, (data)=>{
        $("#app").attr("theme", data["dark-theme"]?"dark":"light");
    });
