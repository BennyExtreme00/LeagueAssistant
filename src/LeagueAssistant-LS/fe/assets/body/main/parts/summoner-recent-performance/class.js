class Main_Part_SummonerRecentPerformance extends AppBodyMain_Part {
    ...
    ReloadContent = ()=>{
        ...
        let optionsChampion = parseInt($(this.element).find(".options-champion .option-champion-item.active").attr("data-id"));
        ...
        let collectingStats = {
            "kda": {
                ...
                "collecting": {
                    ...
                    "extra": (kills, deaths, assists)=>[["kda", ((kills+assists)/Math.max(deaths, 1)).toFixed(1)], ],
                }
            },
            "damage-dealt": {
                "labels": {
        ...
        let displaySummaries = (summaries)=>{
            ...
            window.Widgets.SetupButtonGroup($(this.element).find(".options-champion .option-champion-item"), this.ReloadContent);
