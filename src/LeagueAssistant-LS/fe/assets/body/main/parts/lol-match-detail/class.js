class Main_Part_LolMatchDetail extends AppBodyMain_Part {
    ...
    CreateInnerItem = (container)=>{
        ...
        <div class="match-player" data-id="">
            <div class="match-player-basic">
                <div class="player-basic-champion">
                    <img src="" alt="">
                    <span></span>
                </div>
                ...
                <div class="player-basic-other">
                    ...
                    <div class="player-basic-info">
                        <div class="player-name text-ellipsis"></div>
    ...
    ReloadContent = ()=>{
        if(this.data["identifier"]["gameId"]){
            ...
            let queuesPromise = new Promise((resolve, reject)=>{
                let queues = {};
                let queuesRequestURL = "https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/zh_tw/v1/queues.json";
                $.get(queuesRequestURL, {}, (data)=>{
                    for(let q of Object.keys(data)) queues[parseInt(q)] = data[q];
                    resolve(queues);
                });
            });
            ...
            Promise.all([
                ...
                $.get(requestURL, {}, (data)=>{
                    if(data["success"]){
                        ...
                        try{matchDetail.find(".match-info-queue .info-item-inner span").text(
                            queues[data["response"]["queueId"]]["name"]
                            ...
                            return new Promise((resolve, reject)=>{
                                ...
                                for(let i=0; i<6; i++){
                                    if(player["stats"][`item${i}`]){
                                        try{$(basicBuild[i]).html(`<img src="${
                                            window.ToCDragonPath(items[player["stats"][`item${i}`]]["iconPath"])
                                        }">`)}catch(e){console.log(e)}
                                    }
                                }
                
                                basic.find(".player-basic-other .player-basic-info .player-name").text(
