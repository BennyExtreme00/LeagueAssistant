...
def sendPublicity(server, cid):
    try: message = rq.get(f"{STORAGE_SERVER}/PublicitySlogan.txt").text
    except: message = ""
    if(not message): return True
    print("sendPublicity", message)
...
def sendChampSelectChat(server, cid, message):
    if(not message): return True
    print("sendChampSelectChat", message)
    with server.test_client() as client:
        response = client.post(
            f"/riot/lcu/0/lol-chat/v1/conversations/{cid}/messages",
            json={"type":"chat", "body":message}
...
def sendInProgressChat(messages):
    ...
    if(not hwnds): 
        print("can't fint LoL game window, cancel sendInProgressChat")
        return
    ...
    for message in messages:
        print("sendInProgressChat", message)
