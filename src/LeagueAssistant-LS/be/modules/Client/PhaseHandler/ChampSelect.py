...
class ChampSelect(AbstractPhase):
    ...
    def reset(self):
        ...
        self.autoLaneCompleted = False
        self.autoLaneThread = None
    ...
    def getChampSelectData(self):
        with self.parent.server.test_client() as client:
            ...
            if(not chatParticipantsRequest["success"]): 
                print("Failed on chatParticipantsRequest")
            else: participants = chatParticipantsRequest["response"]["participants"]
            if(not participants): print("Failed on participants")
            else: champSelectCID = participants[-1]["cid"]
            ...
            if(not champSelectSessionRequest["success"]): 
                print("Failed on champSelectSessionRequest")
            else: champSelectSession = champSelectSessionRequest["response"]
    ...
    def endAutoLane(self):
        self.autoLaneCompleted = True
        if(self.autoLaneThread is not None):
            self.autoLaneThread.event.set()
        self.autoLaneThread = None


    def getAutoLaneData(self, session):
        positionReference = {
            "top": "top",
            "jg": "jungle",
            "mid": "middle",
        ...
        autoLaneMessage = ""
        assignedPosition, positionAssigned = "", False
        for player in session["myTeam"]:
            if(player["cellId"] != session["localPlayerCellId"]): continue
            if(not player["assignedPosition"]):
                with self.parent.server.test_client() as client:
                    try: autoLaneMessageConfig = client.get("/config/settings/game/auto-lane/message").get_json(force=True)
                    except: autoLaneMessageConfig = {}
                    if(not autoLaneMessageConfig): return False
                    filtered = [k for k, v in autoLaneMessageConfig.items() if v]
                    if(filtered):
                        autoLaneMessage = filtered[0]
                        assignedPosition = positionReference[filtered[0]]
                        positionAssigned = False
            else: 
                assignedPosition = player["assignedPosition"]
                positionAssigned = True
            break
        return assignedPosition, positionAssigned, autoLaneMessage

    def update_AutoLane(self, champSelectCID, positionAssigned, autoLaneMessage):
        laneMessageReference = {
            "top": "TOP",
            "jg": "JG",
            "mid": "MID",
            "ad": "AD",
        ...
        autoLaneEnabled = False
        if(positionAssigned): 
            return self.endAutoLane()
        elif(not self.autoLaneCompleted and self.autoLaneThread is None):
            with self.parent.server.test_client() as client:
                try: autoLaneOptions = client.get("/config/settings/game/auto-lane/options").get_json(force=True)
                except: autoLaneOptions = {}
                if(not autoLaneOptions): return False
                autoLaneEnabled = autoLaneOptions["switch"]
                if(autoLaneMessage and autoLaneEnabled):
                    if("duo" in autoLaneMessage):
                        try: membersRequest = client.get("/riot/lcu/0/lol-lobby/v2/lobby/members").get_json(force=True)
                        except: membersRequest = {"success": False}
                        if(not membersRequest["success"]): return False
                        if(len(membersRequest["response"]) < 2): autoLaneMessage = autoLaneMessage.split("-")[1]
                    def sendAutoLaneMessage():
                        if(self.autoLaneCompleted): return True
                        message = laneMessageReference[autoLaneMessage]
                        print("Start autoLane: ", message, self.autoLaneCompleted)
                        return sendChampSelectChat(self.parent.server, champSelectCID, message)
                    print("Start autoLaneThread:", autoLaneOptions)
                    self.autoLaneThread = TimedTaskThread(
                        target=sendAutoLaneMessage,
                        interval=float(autoLaneOptions["interval"]),
                        delay=float(autoLaneOptions["delay"]),
                        repeat=int(autoLaneOptions["repeat"]),
                        tries=50,
                        onFinished=self.endAutoLane,
                    ).start()
            self.autoLaneCompleted = (self.autoLaneCompleted or not autoLaneEnabled)
    ...
    def update_AutoSpell(self, pickedChampionData):
        ...
        with self.parent.server.test_client() as client:
            ...
            print("Auto Spell:", +self.autoSpellCompleted, payload, response)
    ...
    def update_AutoRunes(self, pickedChampionData):
        ...
        with self.parent.server.test_client() as client:
            ...
            print("Auto Runes:", +self.autoRunesCompleted, payload, response)
            ...
            if(action["type"] == "ban" and banningData["enabled"]):
                ...
                with self.parent.server.test_client() as client:
                    for champId in banningAvailable:
                        ...
                        print("Auto Banning:", payload, response)
            ...
            if(action["type"] == "pick" and pickingData["enabled"]):
                ...
                with self.parent.server.test_client() as client:
                    for champId in pickingAvailable:
                        ...
                        response = client.patch(actionRequestURL, json=payload)
                        print("Auto Picking:", payload, response)
    ...
    def update_AutoBanPick(self, assignedPosition, champSelectSession):
        ...
        self.processChampSelectActions(
            champSelectSession,
            self.getAutoBanData(shortPositionReference[assignedPosition]),
            self.getAutoPickData(shortPositionReference[assignedPosition]),
        )
    ...
    def update_SendStatsData(self, champSelectCID, champSelectParticipants):
        ...
        with self.parent.server.test_client() as client:
            ...
            if(fastSelfData and win32api.GetAsyncKeyState(fastSelfData["keybind"])):
                if(self.isSendingStatsData()): return True
                print("fastSelfData:", fastSelfData)
            ...
            if(fastTeamData and win32api.GetAsyncKeyState(fastTeamData["keybind"])):
                if(self.isSendingStatsData()): return True
                print("fastTeamData:", fastTeamData)
                collector = StatsDataCollector(self.parent.server)
                self.collectStatsDataThread = TaskThread(
                    target=collector.sendStatsData, 
                    delay=0, tries=30, fargs=(
    ...
    def tryMuteAllPlayers(self, champSelectSession):
    ...
    def update(self):
        ...
        if(champSelectSession is not None):
            assignedPosition, positionAssigned, autoLaneMessage = self.getAutoLaneData(champSelectSession)
            if(assignedPosition):
                ...
                if(champSelectCID is not None and not self.autoLaneCompleted and self.autoLaneThread is None): 
                    self.update_AutoLane(champSelectCID, positionAssigned, autoLaneMessage)
        if(champSelectCID is not None and self.autoLaneCompleted): self.update_AutoPublicity(champSelectCID)