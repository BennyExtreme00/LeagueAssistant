...
class InProgress(AbstractPhase):
    ...
    def getInProgressData(self):
        with self.parent.server.test_client() as client:
            ...
            if(not localNameRequest["success"]): 
                print("Failed on localNameRequest")
            else: localName = localNameRequest["response"]
            ...
            if(localName is not None):
                ...
                if(not playerListRequest["success"]): 
                    print("Failed on playerListRequest")
                else: playerList = playerListRequest["response"]
    ...
    def update_SendStatsData(self, localTeam, playerListByTeamByName):
        ...
        with self.parent.server.test_client() as client:
            ...
            if(fastEnemyData and win32api.GetAsyncKeyState(fastEnemyData["keybind"])):
                if(self.isSendingStatsData()): return True
                print("fastEnemyData:", fastEnemyData)
                collector = StatsDataCollector(self.parent.server)
        ...
        with self.parent.server.test_client() as client:
            for fastType, fastTeam in fastRef.items():
                ...
                if(win32api.GetAsyncKeyState(fastData["keybind"])):
                    collector = StatsDataCollector(self.parent.server)
                    playerNames = [n for n,p in playerListByTeamByName[fastTeam].items() if not p["isBot"]]
