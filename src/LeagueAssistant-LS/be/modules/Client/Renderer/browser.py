...
class BrowserWindow(QWebEngineView):
    ...
    def resize(self, w, h):
        ...
        print((self.width(), self.height()), (w, h), (self.width(), self.height()) == (w, h))
        super().resize(w, h)
        self.centralize()
    ...
    def connect(self, server, host, port):
        self.server = server
        self.server.registerAppControl("app-controls-close", self.closeSignal.emit)
        self.server.registerAppControl("app-controls-hide", self.minimizeSignal.emit)
        self.server.registerAppControl("app-controls-resize", self.resizeSignal.emit)
        self.server.registerAppControl("app-controls-show", self.showSignal.emit)
        self.load(QtCore.QUrl(f"http://{host}:{port}/ui"))
        print(f"Connected to: http://{host}:{port}/ui")
        self.centralize()
