...
import screeninfo, difflib, time, os
...
class SpellHelperPlayer(QWidget):
    ...
    def calculateSpellCastTime(self, key):
        ...
        with self._parent.server.test_client() as client:
            ...
            print(f"recorded {self.data['championName']:>3} {key} cast time (abilityHaste: {abilityHaste:>2})")
            return True
...
class SpellHelperUI(QWidget):
    ...
    def __new__(cls, server, localTeam, gameStats):
        ...
        print("Creating instance of spellHelper")
    ...
    def addPlayer(self, playerData, index):
        ...
        spell1key = self.getSpellKey(playerData["summonerSpells"]["summonerSpellOne"]["rawDisplayName"].split("_")[2])
        ...
        spell2key = self.getSpellKey(playerData["summonerSpells"]["summonerSpellTwo"]["rawDisplayName"].split("_")[2])
