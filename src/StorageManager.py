...
class LocalStorage:
    ...
    def updateFile(self, _path, _name, _type):
        ...
        try:
            response = rq.get(sourcePath.replace("\\", "/"))
            if(response.status_code//100 == 2):
                os.makedirs(os.path.dirname(targetPath), exist_ok=True)
                with open(targetPath, "wb") as f: f.write(response.content)
            else: print("LS-Update failed on", (_path, _name, _type), response)
        except Exception as e: print("LS-Update error on", (_path, _name, _type), e)
    ...
    def setup(self, f_type:str="^.+$", progressCallback=lambda status=None,progress=None:0) -> str:
        ...
        root = ET.fromstring(rq.get(struct.replace("\\", "/")).text)
        ...
        if(updatingStorage): print("Updating storage from", CHVN, "to", LHVN)
        def walk(root, parent, path):
            ...
            if(parent.tag == "folder"):
                ...
                if(sys.kwargs.get("--mode", None) != "DEBUG"):
                ...
                elif(updateCuzStorage): print("LS-Update: Storage", f"{parent.attrib['name']:>15}", f"{parent.attrib['type']:>5}", parent.attrib["path"])
                elif(updateCuzMissing): print("LS-Update: Missing", f"{parent.attrib['name']:>15}", f"{parent.attrib['type']:>5}", parent.attrib["path"])
                elif(updateCuzContent): print("LS-Update: Content", f"{parent.attrib['name']:>15}", f"{parent.attrib['type']:>5}", parent.attrib["path"])
